...
WHERE E.DEPT_CODE=D.DEPT_ID AND E.JOB_CODE=J.JOB_CODE AND EMP_NAME LIKE '전%' AND SUBSTR(EMP_NO,1,1)=7;
-->> ANSI 구문
SELECT EMP_NAME, EMP_NO, DEPT_TITLE, JOB_NAME
FROM EMPLOYEE E
JOIN DEPARTMENT D ON(E.DEPT_CODE=D.DEPT_ID)
JOIN JOB J ON(E.JOB_CODE=J.JOB_CODE)
WHERE EMP_NAME LIKE '전%' AND SUBSTR(EMP_NO,1,1)=7;
...
--> 오라클 전용 구문
SELECT NVL(DEPT_TITLE,'부서없음'), ROUND(AVG(SALARY))
FROM EMPLOYEE E, DEPARTMENT D
WHERE E.DEPT_CODE=D.DEPT_ID(+)
GROUP BY DEPT_TITLE;
-->> ANSI 구문
